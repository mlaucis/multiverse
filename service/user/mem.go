...
func (s *memService) CreatedByDay(ns string, start, end time.Time) (metrics.Timeseries, error) {
	if err := s.Setup(ns); err != nil {
		return nil, err
	}
	...
	for _, u := range us {
		...
		bucket := u.CreatedAt.Format(metrics.BucketFormat)

		if _, ok := bs[bucket]; !ok {
			bs[bucket] = 0
		}
