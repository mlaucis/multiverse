...
type (
	...
	Event interface {
		...
		Create(accountID, applicationID int64, currentUserID string, event *entity.Event, retrieve bool) (evn *entity.Event, err errors.Error)

		// Read returns the event matching the ID or an error
		Read(accountID, applicationID int64, userID, currentUserID, eventID string) (event *entity.Event, err errors.Error)

		// Update updates an event in the database and returns the updated event or an error
		Update(accountID, applicationID int64, currentUserID string, existingEvent, updatedEvent entity.Event, retrieve bool) (evn *entity.Event, err errors.Error)

		// Delete deletes the event matching the IDs or an error
		Delete(accountID, applicationID int64, currentUserID string, evt *entity.Event) errors.Error

		// List returns all events from a certain user
		List(accountID, applicationID int64, userID, currentUserID string) (events []*entity.Event, err errors.Error)
		...
		GeoSearch(accountID, applicationID int64, currentUserID string, latitude, longitude, radius float64, nearest int64) (events []*entity.Event, err errors.Error)

		// ObjectSearch returns all the events for a specific object
		ObjectSearch(accountID, applicationID int64, currentUserID, objectKey string) ([]*entity.Event, errors.Error)

		// LocationSearch returns all the events for a specific object
		LocationSearch(accountID, applicationID int64, currentUserID, locationKey string) ([]*entity.Event, errors.Error)
	}
)
