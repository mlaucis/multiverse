...
const (
	...
	searchApplicationUsersByEmailQuery = `SELECT json_data
	FROM app_%d_%d.users
	WHERE json_data->>'email' IN (?)
		AND (json_data->>'enabled')::BOOL = true
		AND (json_data->>'deleted')::BOOL = false
	LIMIT 200`
  ...
  (SELECT count(*) FROM app_%d_%d.connections
    WHERE (json_data->>'user_from_id')::BIGINT = $1::BIGINT AND
          (json_data->>'enabled')::BOOL = true AND
          json_data->>'type' = '` + entity.ConnectionTypeFriend + `') AS "friends",
  (SELECT count(*) FROM app_%d_%d.connections
    WHERE (json_data->>'user_to_id')::BIGINT = $1::BIGINT AND
          (json_data->>'enabled')::BOOL = true AND
          json_data->>'type' = '` + entity.ConnectionTypeFollow + `') AS "follower",
  (SELECT count(*) FROM app_%d_%d.connections
    WHERE (json_data->>'user_from_id')::BIGINT = $1::BIGINT AND
          (json_data->>'enabled')::BOOL = true AND
          json_data->>'type' = '` + entity.ConnectionTypeFollow + `') AS "followed"`
)
