...
func VerifyRequest(ctx *context.Context, numKeyParts int) errors.Error {
	encodedIds := ctx.R.Header.Get("x-tapglue-app-key")
	decodedIds, err := utils.Base64Decode(encodedIds)
	if err != nil {
		return errors.NewBadRequestError("application key validation failed (1)", err.Error())
	}

	ids := strings.SplitN(string(decodedIds), ":", numKeyParts)
	if len(ids) != numKeyParts {
		return errors.NewBadRequestError("application key validation failed (2)", fmt.Sprintf("app key parts count mismatch expected %d got %d", numKeyParts, len(ids)))
	}

	accountID, err := strconv.ParseInt(ids[0], 10, 64)
	if err != nil {
		return errors.NewBadRequestError("application key validation failed (3)", "failed to parse account id")
	}

	if accountID != ctx.AccountID {
		return errors.NewBadRequestError("application key validation failed (4)", fmt.Sprintf("account id mismatch expected %d got %d", ctx.AccountID, accountID))
	}
	...
	if numKeyParts == 1 {
		...
		if er != nil {
			return errors.NewBadRequestError("application key validation failed (5)", er.Error())
		}
		authToken = account.AuthToken
	} else {
		applicationID, er := strconv.ParseInt(ids[1], 10, 64)
		if er != nil {
			return errors.NewBadRequestError("application key validation failed (6)", er.Error())
		}

		if applicationID != ctx.ApplicationID {
			return errors.NewBadRequestError("application key validation failed (7)", fmt.Sprintf("app id mismatch expected %d got %d", ctx.ApplicationID, applicationID))
		}

		application, err := core.ReadApplication(accountID, applicationID)
		if err != nil {
			return errors.NewBadRequestError("application key validation failed (8)", er.Error())
		}
	...
	*/
	...
	return errors.NewBadRequestError("application key validation failed (9)\nsignature mismatch",
